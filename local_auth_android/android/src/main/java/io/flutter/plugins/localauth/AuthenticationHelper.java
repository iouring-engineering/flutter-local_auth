...
class AuthenticationHelper extends BiometricPrompt.AuthenticationCallback
    ...
    AuthenticationHelper(
        ...
        final List<String> authenticators = ((List<String>) call.argument("allowedAuthenticators"));
        int authenticatorTypes = 0;
        for (int i = 0; i < authenticators.size(); i++) {
            if (authenticators.size() >= i + 1) {
                if (authenticatorTypes == 0) {
                    authenticatorTypes = getAuthenticatorType(authenticators.get(i));
                } else {
                    authenticatorTypes |= getAuthenticatorType(authenticators.get(i));
                }
            }
    ...
    int getAuthenticatorType(String authenticatorString) {
